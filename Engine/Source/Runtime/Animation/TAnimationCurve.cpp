...
template <class T>
TAnimationCurve<T>::TAnimationCurve(const std::vector<KeyFrame>& keyframes)
...
T TAnimationCurve<T>::evaluate(float time, const TCurveCache<T>& cache, bool loop) const
{
	...
	if (Mathf::CompareApproximately(mLength, 0.0f))
		time = 0.0f;
	...
	cubicHermiteCoefficients(leftKey.value, rightKey.value, leftKey.outTangent, rightKey.inTangent, length,
...
TKeyframe<T> TAnimationCurve<T>::evaluateKey(const KeyFrame& lhs, const KeyFrame& rhs, float time) const
{
	...
	TKeyframe<T> output;
	output.time = time;
	output.value = cubicHermite(t, lhs.value, rhs.value, leftTangent, rightTangent);
	output.inTangent = cubicHermiteD1(t, lhs.value, rhs.value, leftTangent, rightTangent) * invLength;
...
TAnimationCurve<T> TAnimationCurve<T>::split(float start, float end)
{
	...
	start = Mathf::Clamp(start, mStart, mEnd);
	end = Mathf::Clamp(end, mStart, mEnd);

	if (Mathf::CompareApproximately(end - start, 0.0f))
	...
	if (!Mathf::CompareApproximately(startKey.time, start))
	{
	...
	if (!Mathf::CompareApproximately(endKey.time, end))
	{
