...
namespace FishEngine
{
	...
	AnimationClip::AnimationClip()
		: mVersion(0), mCurves(bs_shared_ptr_new<AnimationCurves>())
	...
	AnimationClip::AnimationClip(const SPtr<AnimationCurves>& curves, bool isAdditive, UINT32 sampleRate,
		...
		: mVersion(0), mCurves(curves), mRootMotion(rootMotion), mIsAdditive(isAdditive), mLength(0.0f)
	...
	void AnimationClip::buildNameMapping()
	{
		...
		{
			std::vector<TNamedAnimationCurve<float>>& curve = mCurves->generic;
			for (UINT32 i = 0; i < (UINT32)curve.size(); i++)
			{
				auto& entry = curve[i];
