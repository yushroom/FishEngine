...
namespace FishEngine
{
	...
	Skeleton::Skeleton(BONE_DESC* bones, UINT32 numBones)
		:mInvBindPoses(numBones), mBoneInfo(numBones), mNumBones(numBones)
	{
	...
	{
		return std::make_shared<Skeleton>(bones, numBones);
	}
		...
		const AnimationClip& clip, float time, bool loop)
	{
		//bs_frame_mark();
		{
			std::vector<AnimationCurveMapping> boneToCurveMapping(mNumBones);
			...
			std::vector<TCurveCache<Vector3>> positionCache(state.curves->position.size());
			std::vector<TCurveCache<Quaternion>> rotationCache(state.curves->rotation.size());
			std::vector<TCurveCache<Vector3>> scaleCache(state.curves->scale.size());
	...
	void Skeleton::getPose(Matrix4x4* pose, LocalSkeletonPose& localPose, const SkeletonMask& mask, 
		const AnimationStateLayer* layers, UINT32 numLayers)
	{
		...
		for(UINT32 i = 0; i < mNumBones; i++)
		{
			...
			localPose.rotations[i] = Quaternion(0, 0, 0, 0);
		...
		for(UINT32 i = 0; i < numLayers; i++)
		{
			...
			for (UINT32 j = 0; j < layer.numStates; j++)
			{
				...
				if (Mathf::CompareApproximately(normWeight, 0.0f))
					continue;
				...
				for (UINT32 k = 0; k < mNumBones; k++)
				{
					...
					if (curveIdx != (UINT32)-1)
					{
						...
						localPose.scales[k].Scale(curve.evaluate(state.time, state.scaleCaches[curveIdx], state.loop) * normWeight);
				...
				if(layer.additive)
				{
					for (UINT32 k = 0; k < mNumBones; k++)
					{
						...
						if (curveIdx != (UINT32)-1)
						{
							...
							value = Quaternion::Lerp(Quaternion::identity, value, normWeight);
				...
				else
				{
					for (UINT32 k = 0; k < mNumBones; k++)
					{
						...
						if (curveIdx != (UINT32)-1)
						{
							...
							if (Quaternion::Dot( value, localPose.rotations[k]) < 0.0f)
								value = -value;
		...
		bool* isGlobal = new bool[isGlobalBytes];
		...
		for(UINT32 i = 0; i < mNumBones; i++)
		{
			...
			if (!isAssigned)
				...
				localPose.rotations[i].NormalizeSelf();
			...
			pose[i] = Matrix4x4::TRS(localPose.positions[i], localPose.rotations[i], localPose.scales[i]);
		}
