...
namespace FishEngine
{
	...
	inline std::string ToString(GUID const& u)
	{
		std::string result;
		//result.reserve(36);
		result.reserve(32);
	...
	std::basic_ostream<ch, char_traits>& operator<<(std::basic_ostream<ch, char_traits> &os, GUID const& u)
	{
		...
		if (ok)
		{
			...
			std::size_t i = 0;
			for (auto i_data = u.begin(); i_data != u.end(); ++i_data, ++i)
			{
				os.width(2);
	...
	template <typename ch, typename char_traits>
	std::basic_istream<ch, char_traits>& operator >> (std::basic_istream<ch, char_traits> &is, GUID &u)
	{
